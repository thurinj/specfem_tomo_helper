#!/usr/bin/env python
# ------------------------------------------------------------------------------
# SPECfem Tomography Helper - Run from Config (User-friendly Script)
# The intent of this script is to provide a test-bed for tinkering with the
# workflow and to allow users to tune it to their needs.
# ------------------------------------------------------------------------------
import sys
import os
import argparse
...
def main():
    ...
    if args.create_config:
        ...
        template_path = os.path.join(os.path.dirname(__file__), f'../specfem_tomo_helper/templates/{template_name}')
    ...
    is_aniso = is_anisotropic_model(variables)
    var_arrays = []
    var_names = []
    if is_aniso:
